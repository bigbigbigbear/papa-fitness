            ...
            <div class="form-section--bd clearfix">
              <el-col :span="12">
                <el-form-item label="生日" prop="birthday">
                  <el-date-picker v-model="form.birthday" format="yyyy-MM-dd" value-format="yyyy-MM-dd" type="date" placeholder="请选择日期"></el-date-picker>
                </el-form-item>
                ...
                <el-form-item label="家庭地址" prop="home_area">
                  <area-select v-model="selectedHome" :level="2" size="large" :placeholders="placeholders" :data="pcaa" @change="getHomeArea"></area-select>
                </el-form-item>
                <el-form-item label="" prop="home_address">
                  <el-input v-model="form.home_address" placeholder="请输入会员家庭地址"></el-input>
                </el-form-item>
                <el-form-item label="工作地址" prop="company_area">
                  <area-select v-model="selectedCompany" :level="2" size="large" :placeholders="placeholders" :data="pcaa" @change="getCompanyArea"></area-select>
                </el-form-item>
                <el-form-item label="" prop="company_address">
                  <el-input v-model="form.company_address" placeholder="请输入会员工作地址"></el-input>
                </el-form-item>
            ...
            <div class="form-section--bd clearfix">
              <el-col :span="12">
                ...
                <el-form-item label="标签" prop="tag_ids">
                  <div :class="['tag',item.selected ? 'active':'']" v-for="(item, index) in tagsList" :key="index" @click="selectTags(item)">{{item.tag_name}}</div>
                </el-form-item>
              </el-col>
            </div>
          </el-col>
...
import { uploadImg, getBaseTags } from '@/api/common'
import { dataURItoBlob } from '@/utils'
import { stringToNumber } from '@/utils/filters'
  ...
  methods: {
    getMemInfo() {
      ...
      getMemberInfo(params).then(res => {
        ...
        this.fileList = avatars
        for (const k in data) {
          if (data[k] === 0) {
            data[k] = ''
          }
          if (typeof data[k] === 'number' && k !== 'sex') {
    ...
    getHomeArea(val) {
      console.log(val)
      this.home_area = val[2]
    },
    // 获取公司区域
    getCompanyArea(val) {
      console.log(val)
      this.company_area = val[2]
    },
    getTagList() {
      const params = {
        page: 1,
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          params.referrer_id = stringToNumber(params.referrer_id)
          params.personnel_id = stringToNumber(params.personnel_id)
          params.instructor_id = stringToNumber(params.instructor_id)
          params.from_id = stringToNumber(params.from_id)
          params.uid = uid
