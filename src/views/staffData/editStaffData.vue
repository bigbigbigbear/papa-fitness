...
import { getStaffData, editStaffData } from '@/api/staffData'
...
export default {
  ...
  methods: {
    ...
    setStaffData(result) {
      ...
      this.ruleForm.is_allowed_login = '' + this.ruleForm.is_allowed_login + ''
      this.ruleForm.position_id = result.position_ids
      result.position_ids.forEach((value, key) => {
        this.ruleForm.position_id[key] = value + ''
      })
      this.ruleForm.gym_id = result.gym_ids
      this.ruleForm.gym_id.forEach((value, key) => {
        this.ruleForm.gym_id[key] = value + ''
      })
      this.selected.push(this.ruleForm.province_id + '')
      this.selected.push(this.ruleForm.city_id + '')
      this.selected.push(this.ruleForm.county_id + '')
      this.fileList.push(this.ruleForm.avatar)
    },
    // 切换头像上传方式
    changeUploadWay(val) {
      if (val === 1) {
    ...
    addData() {
      ...
      editStaffData(this.ruleForm).then((result) => {
        if (!result.is_error) {
          this.$message({
            message: '编辑成功！',
            type: 'success',
